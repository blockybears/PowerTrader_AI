...
# -----------------------------
# Exchange market data via CCXT
# -----------------------------
_CCXT_EXCHANGE = None  # lazy-init so import doesn't explode if .env missing


def get_current_ask(symbol: str) -> float:
    """
    Returns current BUY price (ask) for symbols like 'BTC-USD'.
    Uses CCXT exchange configured in .env.
    """
    ...
    if _CCXT_EXCHANGE is None:
        ...
        _CCXT_EXCHANGE = ExchangeInterface(env_path)

    ticker = _CCXT_EXCHANGE.fetch_ticker(symbol)
    return ticker["ask"]
...
def step_coin(sym: str):
	...
	if tf_choice_index >= len(tf_choices):
		...
		# get current price ONCE per coin — use CCXT exchange's current ASK (same as trader buy price)
		rh_symbol = f"{sym}-USD"
		while True:
			try:
				current = get_current_ask(rh_symbol)  # Changed from robinhood_current_ask
				break
			except Exception:
				time.sleep(1)
				continue
